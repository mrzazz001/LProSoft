...
{
    public partial  class FrmSecretariatTyp : Form
    { void avs(int arln)

{ 
 panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; label40.Text=   (arln == 0 ? "  إسم النوع - انجليزي :  " : "  Genre name - English:") ; label36.Text=   (arln == 0 ? "  إسم النوع - عربـــي :  " : "  Type name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; Text = "كــــــرت أنواع العهـــد";this.Text=   (arln == 0 ? "  كــــــرت أنواع العهـــد  " : "  Types of vows") ;}
...
#pragma warning disable CS0414 // The field 'FrmSecretariatTyp.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmSecretariatTyp.FlagUpdate' is assigned but its value is never used
        ...
        private T_SecretariatsTyp data_this;
        ...
        public T_SecretariatsTyp DataThis
        {
            get
            {
                return data_this;
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_SecretariatsTyp> list = db.FillSecretariatsTyp_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_SecretariatsTyp> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_SecretariatsTyp";
        ...
        public void Clear()
        {
            ...
            State = FormState.New;
            data_this = new T_SecretariatsTyp();
            State = FormState.New;
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_SecretariatsTyps.Select((T_SecretariatsTyp item) => new
            {
                Code = item.SecretariatTyp_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmSecretariatTyp));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
                ...
                textBox_Note.WatermarkText = "ملاحظـــــــات النـــوع";
                Text = "كرت أنواع العهــد";
            }
            else
            {
        ...
        private void FrmSecretariatTyp_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmSecretariatTyp));
                ...
                {
                    columns_Names_visible.Add("SecretariatTyp_No", new ColumnDictinary("رقم النوع", "Type No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_SecretariatsTyp newData = db.SecretariatsTypEmp(no);
                if (newData == null || newData.SecretariatTyp_No == 0 || string.IsNullOrEmpty(newData.SecretariatTyp_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.SecretariatTyp_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.SecretariatTyp_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
                ...
                try
                {
                    if (data_this.SecretariatTyp_ID == "1")
                    {
                        IfDelete = false;
                    }
                    else
        ...
        public void SetData(T_SecretariatsTyp value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.SecretariatTyp_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_SecretariatsTyp GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.SecretariatTyp_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.SecretariatTyp_ID = textBox_ID.Tag.ToString();
                        db.T_SecretariatsTyps.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                    catch (SqlException ex3)
                    {
                        ...
                        data_this.SecretariatTyp_No = max;
                        Button_Save_Click(sender, e);
                    }
                    catch (Exception)
                    {
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.SecretariatTyp_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.SecretariatTyp_No == 0 || string.IsNullOrEmpty(data_this.SecretariatTyp_ID) || !ifOkDelete)
            {
                return;
            }
            T_Secretariat returned = db.SelectSecretariatsTypByReturnNo(DataThis.SecretariatTyp_No);
            int num;
            if (returned != null)
            {
                int? secretariatsTyp = returned.SecretariatsTyp;
                num = ((secretariatsTyp.Value == 0 && secretariatsTyp.HasValue) ? 1 : 0);
            }
            else
            {
                num = 1;
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف الوظيفة .. لانه مرتبط باحد الموظفين" : "can not be deleted This Job .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.SecretariatsTypEmp(DataThis.SecretariatTyp_No);
            try
            {
                db.T_SecretariatsTyps.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_SecretariatsTyp")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["SecretariatTyp_No"].Width = 120;
            panel.Columns["SecretariatTyp_No"].Visible = columns_Names_visible["SecretariatTyp_No"].IfDefault;
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير أنواع العهــد");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_SecretariatsTyp.SecretariatTyp_No as No, T_SecretariatsTyp.NameA as NmA, T_SecretariatsTyp.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
