...
{
    public partial  class FrmCarTypS : Form
    { void avs(int arln)

{ 
 label40.Text=   (arln == 0 ? "  إسم النوع - انجليزي :  " : "  Genre name - English:") ; label36.Text=   (arln == 0 ? "  إسم النوع - عربـــي :  " : "  Type name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */Text = "كــــــرت أنواع السيارات";this.Text=   (arln == 0 ? "  كــــــرت أنواع السيارات  " : "  car types card") ;}
...
#pragma warning disable CS0414 // The field 'FrmCarTypS.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmCarTypS.FlagUpdate' is assigned but its value is never used
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_CarTyp> list = db.FillCarTyp_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_CarTyp> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_CarTyp";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_CarTyps.Select((T_CarTyp item) => new
            {
                Code = item.CarTyp_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmCarTypS));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
                ...
                textBox_Note.WatermarkText = "ملاحظـــــــات الماركة";
                Text = "كرت ماركات السيارات";
            }
            else
            {
        ...
        private void FrmCarTypS_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmCarTypS));
                ...
                {
                    columns_Names_visible.Add("CarTyp_No", new ColumnDictinary("رقم الماركة", "Type No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_CarTyp newData = db.CarTypEmp(no);
                if (newData == null || newData.CarTyp_No == 0 || string.IsNullOrEmpty(newData.CarTyp_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.CarTyp_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.CarTyp_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_CarTyp value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.CarTyp_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_CarTyp GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.CarTyp_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.CarTyp_ID = textBox_ID.Tag.ToString();
                        db.T_CarTyps.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.CarTyp_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.CarTyp_No == 0 || string.IsNullOrEmpty(data_this.CarTyp_ID) || !ifOkDelete)
            {
                return;
            }
            T_Car returned = db.SelectCarTypByReturnNo(DataThis.CarTyp_No);
            int num;
            if (returned != null)
            {
                int? carType = returned.CarType;
                num = ((carType.Value == 0 && carType.HasValue) ? 1 : 0);
            }
            else
            {
                num = 1;
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف الماركة .. لانه مرتبط باحد الموظفين" : "can not be deleted This Type .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.CarTypEmp(DataThis.CarTyp_No);
            try
            {
                db.T_CarTyps.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_CarTyp")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["CarTyp_No"].Width = 120;
            panel.Columns["CarTyp_No"].Visible = columns_Names_visible["CarTyp_No"].IfDefault;
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير أنواع السيارات");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_CarTyp.CarTyp_No as No, T_CarTyp.NameA as NmA, T_CarTyp.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
