...
{ 
 panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; label8.Text=   (arln == 0 ? "  اللغة الافتراضية :  " : "  default language:") ; label2.Text=   (arln == 0 ? "  كلمة المرور :  " : "  password :") ; label40.Text=   (arln == 0 ? "  إسم النادل - انجليزي :  " : "  Waiter Name - English:") ; label36.Text=   (arln == 0 ? "  إسم النادل - عربي :  " : "  Waiter Name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; Text = "كرت نادلين المطعم";this.Text=   (arln == 0 ? "  كرت نادلين المطعم  " : "  Restaurant waiters card") ;}
...
#pragma warning disable CS0414 // The field 'Frmwaiter.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'Frmwaiter.FlagUpdate' is assigned but its value is never used
        ...
        public T_User Permmission
        {
            ...
            set
            {
                ...
                {
                    permission = value;
                    if (!VarGeneral.TString.ChkStatShow(value.FilStr, 45))
                    {
                        IfAdd = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.FilStr, 46))
                    {
                        CanEdit = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.FilStr, 47))
                    {
                        IfDelete = false;
                    }
                    else
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_Waiter> list = db.FillWaiter_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_Waiter> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_Waiter";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_Waiters.Select((T_Waiter item) => new
            {
                Code = item.waiter_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(Frmwaiter));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void Frmwaiter_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    columns_Names_visible.Add("waiter_No", new ColumnDictinary("رقم النادل", "Waiter No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_Waiter newData = db.StockWaiter(no.ToString());
                if (newData == null || newData.waiter_ID == 0)
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.waiter_No;
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(newData.waiter_No ?? "");
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_Waiter value)
        {
            try
            {
                State = FormState.Saved;
                Button_Save.Enabled = false;
                textBox_ID.Tag = value.waiter_ID;
                textBox_NameA.Text = value.Arb_Des;
                textBox_NameE.Text = value.Eng_Des;
                txtPass.Text = value.Pass;
        ...
        private T_Waiter GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.waiter_No = textBox_ID.Text;
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        db.T_Waiters.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                    catch (SqlException ex3)
                    {
                        ...
                        data_this.waiter_No = string.Concat(max);
                        Button_Save_Click(sender, e);
                    }
                    catch (Exception)
                    {
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(data_this.waiter_No ?? "") + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.waiter_ID == 0 || !ifOkDelete)
            {
                return;
            }
            T_Room returned = db.SelectWaiterNoByReturnNo(DataThis.waiter_ID);
            if (returned != null && returned.waiterNo.HasValue)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف هذا النادل .. لانه مرتبط باحد الطاولات" : "You can not delete Waiter .. because it is tied to a Tables", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.StockWaiter(DataThis.waiter_No.ToString());
            try
            {
                db.T_Waiters.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_Waiter")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["waiter_No"].Width = 200;
            panel.Columns["waiter_No"].Visible = columns_Names_visible["waiter_No"].IfDefault;
            panel.ReadOnly = true;
        }
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير النادل");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _RepShow.Tables = " T_Waiter left join T_SYSSETTING on T_SYSSETTING.SYSSETTING_ID = T_Waiter.CompanyID ";
                string Fields = "";
                Fields = " T_Waiter.waiter_No as No, T_Waiter.Arb_Des as NmA, T_Waiter.Eng_Des as NmE,T_SYSSETTING.LogImg ";
                _RepShow.Rule = "";
