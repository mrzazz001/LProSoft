...
{
    public partial  class FrmBirthPlace : Form
    { void avs(int arln)

{ 
 panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; label40.Text=   (arln == 0 ? "  إسم المكان - انجليزي :  " : "  Place name - English:") ; label36.Text=   (arln == 0 ? "  إسم المكان - عربـــي :  " : "  Place name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; Text = "كــــــرت  أماكن الميـــلاد";this.Text=   (arln == 0 ? "  كــــــرت  أماكن الميـــلاد  " : "  Cards of Birth Places") ;}
...
#pragma warning disable CS0414 // The field 'FrmBirthPlace.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmBirthPlace.FlagUpdate' is assigned but its value is never used
        ...
        private T_BirthPlace data_this;
        ...
        public void Button_Search_Click(object sender, EventArgs e)
        {
            ...
            VarGeneral.SFrmTyp = "T_BirthPlace";
            frm.TopMost = true;
            frm.ShowDialog();
            if (frm.SerachNo != "")
            {
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_BirthPlace> list = db.FillBirthPlace_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_BirthPlace> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_BirthPlace";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_BirthPlaces.Select((T_BirthPlace item) => new
            {
                Code = item.BirthPlaceNo + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmBirthPlace));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            ...
            else
            {
                ...
                Text = "Birth Places Card";
            }
        }
        private void FrmBirthPlace_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmBirthPlace));
                ...
                {
                    columns_Names_visible.Add("BirthPlaceNo", new ColumnDictinary("رقم مكان الميلاد", "Birth Palce No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_BirthPlace newData = db.BirthPlaceEmp(no);
                if (newData == null || newData.BirthPlaceNo == 0)
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.BirthPlaceNo.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.BirthPlaceNo));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_BirthPlace value)
        {
            try
            {
                State = FormState.Saved;
        ...
        private T_BirthPlace GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.BirthPlaceNo = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        db.T_BirthPlaces.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.BirthPlaceNo)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.BirthPlaceNo == 0 || !ifOkDelete)
            {
                return;
            }
            T_Emp returned = db.SelectBirthPlaceNoByReturnNo(DataThis.BirthPlaceNo);
            int num;
            if (returned != null)
            {
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف مكان الميلاد .. لانه مرتبط باحد الموظفين" : "can not be deleted This Birth Place .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.BirthPlaceEmp(DataThis.BirthPlaceNo);
            try
            {
                db.T_BirthPlaces.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_BirthPlace")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["BirthPlaceNo"].Width = 120;
            panel.Columns["BirthPlaceNo"].Visible = columns_Names_visible["BirthPlaceNo"].IfDefault;
            panel.ReadOnly = true;
        }
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير أماكن الميـــلاد");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_BirthPlace.BirthPlaceNo as No, T_BirthPlace.NameA as NmA, T_BirthPlace.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
