...
{
    public partial  class FrmDriv : Form
    { void avs(int arln)

{ 
 panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; label40.Text=   (arln == 0 ? "  إسم المسؤول - انجليزي :  " : "  Administrator's name - English:") ; label36.Text=   (arln == 0 ? "  إسم المسؤول - عربي :  " : "  Administrator's name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; Text = "كرت مسؤولين التوصيل";this.Text=   (arln == 0 ? "  كرت مسؤولين التوصيل  " : "  Delivery officials card") ;}
...
#pragma warning disable CS0414 // The field 'FrmDriv.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmDriv.FlagUpdate' is assigned but its value is never used
        ...
        public T_User Permmission
        {
            ...
            set
            {
                ...
                {
                    permission = value;
                    if (!VarGeneral.TString.ChkStatShow(value.FilStr, 49))
                    {
                        IfAdd = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.FilStr, 50))
                    {
                        CanEdit = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.FilStr, 51))
                    {
                        IfDelete = false;
                    }
                    else
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_Chauffeur> list = db.FillDriv_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_Chauffeur> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_Chauffeur";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_Chauffeurs.Select((T_Chauffeur item) => new
            {
                Code = item.chauffeur_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmDriv));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            ...
            else
            {
                ...
                Text = "Delivery Officer Card";
            }
        }
        private void FrmDriv_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmDriv));
                ...
                {
                    columns_Names_visible.Add("chauffeur_No", new ColumnDictinary("رقم المسؤول", "admin No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_Chauffeur newData = db.StockDriv(no.ToString());
                if (newData == null || newData.chauffeur_ID == 0)
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.chauffeur_No;
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(newData.chauffeur_No ?? "");
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_Chauffeur value)
        {
            try
            {
                State = FormState.Saved;
                Button_Save.Enabled = false;
                textBox_ID.Tag = value.chauffeur_ID;
        ...
        private T_Chauffeur GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.chauffeur_No = textBox_ID.Text;
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        db.T_Chauffeurs.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(data_this.chauffeur_No ?? "") + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.chauffeur_ID == 0 || !ifOkDelete)
            {
                return;
            }
            T_INVHED returned = db.SelectDrivNoByReturnNo(DataThis.chauffeur_ID);
            if (returned != null && returned.chauffeurNo.HasValue)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف هذا المسؤول .. لانه مرتبط باحد الفواتير" : "You can not delete Driver .. because it is tied to a Invoices", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.StockDriv(DataThis.chauffeur_No.ToString());
            try
            {
                db.T_Chauffeurs.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_Chauffeur")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["chauffeur_No"].Width = 200;
            panel.Columns["chauffeur_No"].Visible = columns_Names_visible["chauffeur_No"].IfDefault;
            panel.ReadOnly = true;
        }
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير مسؤولين التوصيل");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _RepShow.Tables = " T_Chauffeur left join T_SYSSETTING on T_SYSSETTING.SYSSETTING_ID = T_Chauffeur.CompanyID ";
                string Fields = "";
                Fields = " T_Chauffeur.chauffeur_No as No, T_Chauffeur.Arb_Des as NmA, T_Chauffeur.Eng_Des as NmE,T_SYSSETTING.LogImg ";
                _RepShow.Rule = "";
