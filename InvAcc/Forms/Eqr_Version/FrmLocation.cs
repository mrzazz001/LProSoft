...
#pragma warning disable CS0414 // The field 'FrmLocation.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmLocation.FlagUpdate' is assigned but its value is never used
       ...
       // private IContainer components = null;
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_Loc> list = db.FillLoc_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_Loc> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_Loc";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_Locs.Select((T_Loc item) => new
            {
                Code = item.Loc_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmLocation));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void FrmLocation_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    columns_Names_visible.Add("Loc_No", new ColumnDictinary("الرقـــم", "No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_Loc newData = db.StockLoc(no.ToString());
                if (newData == null || newData.Loc_ID == 0)
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.Loc_No;
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(newData.Loc_No ?? "");
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_Loc value)
        {
            try
            {
                State = FormState.Saved;
                Button_Save.Enabled = false;
                textBox_ID.Tag = value.Loc_ID;
        ...
        private T_Loc GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.Loc_No = textBox_ID.Text;
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        db.T_Locs.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                    catch (SqlException ex3)
                    {
                        ...
                        data_this.Loc_No = string.Concat(max);
                        Button_Save_Click(sender, e);
                    }
                    catch (Exception)
                    {
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(data_this.Loc_No ?? "") + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.Loc_ID == 0 || !ifOkDelete)
            {
                return;
            }
            T_Rom returned = db.SelectLocNoByReturnNo(DataThis.Loc_ID);
            if (returned != null && returned.aline.HasValue)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف الأتجاه .. لانه مرتبط باحد الغرف" : "You can not delete Direction .. because it is tied to a Rooms", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.StockLoc(DataThis.Loc_No.ToString());
            try
            {
                db.T_Locs.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_Loc")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["Loc_No"].Width = 200;
            panel.Columns["Loc_No"].Visible = columns_Names_visible["Loc_No"].IfDefault;
            panel.ReadOnly = true;
        }
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير اتجاهات الغرف");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _RepShow.Tables = " T_Loc left join T_SYSSETTING on T_SYSSETTING.SYSSETTING_ID = T_Loc.CompanyID ";
                string Fields = "";
                Fields = " T_Loc.Loc_No as No, T_Loc.Arb_Des as NmA, T_Loc.Eng_Des as NmE,T_SYSSETTING.LogImg ";
                _RepShow.Rule = "";
