...
{ 
 panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; label40.Text=   (arln == 0 ? "  إسم المدينة - انجليزي :  " : "  City name - English:") ; label36.Text=   (arln == 0 ? "  إسم المدينة - عربـــي :  " : "  City name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; Text = "كــــــرت المــدن";this.Text=   (arln == 0 ? "  كــــــرت المــدن  " : "  city ​​card") ;}
...
#pragma warning disable CS0414 // The field 'FrmCity.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmCity.FlagUpdate' is assigned but its value is never used
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_City> list = db.FillCity_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_City> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_City";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_Cities.Select((T_City item) => new
            {
                Code = item.City_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmCity));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
                ...
                textBox_Note.WatermarkText = "ملاحظـــــــات المدينة";
                Text = "كرت المــــدن";
            }
            else
            {
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_City newData = db.CityEmp(no);
                if (newData == null || newData.City_No == 0 || string.IsNullOrEmpty(newData.City_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.City_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.City_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_City value)
        {
            try
            {
                State = FormState.Saved;
        ...
        private T_City GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.City_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.City_ID = textBox_ID.Tag.ToString();
                        db.T_Cities.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.City_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.City_No == 0 || string.IsNullOrEmpty(data_this.City_ID) || !ifOkDelete)
            {
                return;
            }
            T_Emp returned = db.SelectCityNoByReturnNo(DataThis.City_No);
            int num;
            if (returned != null)
            {
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف المدينة .. لانه مرتبط باحد الموظفين" : "can not be deleted This Job .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.CityEmp(DataThis.City_No);
            try
            {
                db.T_Cities.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_City")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["City_No"].Width = 120;
            panel.Columns["City_No"].Visible = columns_Names_visible["City_No"].IfDefault;
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير المــدن");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_City.City_No as No, T_City.NameA as NmA, T_City.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
