...
{ 
        ...
        private void ArbEng()
        {
            ...
            {
                LangArEn = 0;
                FlxPayment.Cols[1].Caption = "المبلــغ";
                FlxPayment.Cols[2].Caption = "شهر السداد";
                FlxPayment.Cols[3].Caption = "حالة السداد";
                FlxPayment.Cols[4].Caption = "المتبقي";
                FlxPayment.Cols[5].Caption = "السند";
                FlxPayment.Cols[6].Caption = "التسلسل";
                FlxPayment.Cols[7].Caption = (SelectPayment ? "إختيار الدفعة" : "حذف الدفعة");
            }
            else
            {
                LangArEn = 1;
        ...
        private void FillCombo()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
            {
                CmbTenanPayMethod.Items.Add("كل شهر");
                CmbTenanPayMethod.Items.Add("كل 2 شهر");
                CmbTenanPayMethod.Items.Add("كل 3 شهور");
                CmbTenanPayMethod.Items.Add("كل 4 شهور");
                CmbTenanPayMethod.Items.Add("كل 6 شهور");
                CmbTenanPayMethod.Items.Add("كل سنة");
            }
            else
            {
        ...
        private void FlxPayment_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (!FrmTyp && e.KeyCode == Keys.Delete && (!(double.Parse(FlxPayment.GetData(FlxPayment.Row, 1).ToString()) > 0.0) || !VarGeneral.CheckDate(FlxPayment.GetData(FlxPayment.Row, 2).ToString()) || MessageBox.Show("هل أنت متاكد من حذف السطر [" + FlxPayment.Row + "]؟ \n Are you sure that you want to delete the line [" + FlxPayment.Row + "]?", VarGeneral.ProdectNam, MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.No) && !Convert.ToBoolean(FlxPayment.GetData(FlxPayment.Row, 3).ToString()))
                {
        ...
        private void FlxPayment_AfterEdit(object sender, RowColEventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    if (double.Parse(FlxPayment.GetData(e.Row, 1).ToString()) > 0.0 && VarGeneral.CheckDate(FlxPayment.GetData(e.Row, 2).ToString()))
                    {
                        if (MessageBox.Show("هل أنت متاكد من حذف السطر [" + e.Row + "]؟ \n Are you sure that you want to delete the line [" + e.Row + "]?", VarGeneral.ProdectNam, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                        {
                            FlxPayment.SetData(e.Row, e.Col, false);
                        }
                        else if (!Convert.ToBoolean(FlxPayment.GetData(e.Row, 3).ToString()))
