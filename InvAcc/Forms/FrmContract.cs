...
#pragma warning disable CS0414 // The field 'FrmContract.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmContract.FlagUpdate' is assigned but its value is never used
       ...
       // private IContainer components = null;
        ...
        public T_User Permmission
        {
            ...
            set
            {
                ...
                {
                    permission = value;
                    if (!VarGeneral.TString.ChkStatShow(value.Emp_FilStr, 29))
                    {
                        IfAdd = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.Emp_FilStr, 30))
                    {
                        CanEdit = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.Emp_FilStr, 31))
                    {
                        IfDelete = false;
                    }
                    else
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_Contract> list = db.FillContract_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_Contract> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_Contract";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_Contracts.Select((T_Contract item) => new
            {
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmContract));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
                ...
                textBox_Note.WatermarkText = "ملاحظـــــــات العقد";
                Text = "كرت العقــــود";
            }
            else
            {
        ...
        private void FrmContract_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    columns_Names_visible.Add("Contract_No", new ColumnDictinary("رقم العقد", "Contract No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_Contract newData = db.ContractEmp(no);
                if (newData == null || newData.Contract_No == 0 || string.IsNullOrEmpty(newData.Contract_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.Contract_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.Contract_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_Contract value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.Contract_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_Contract GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.Contract_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.Contract_ID = textBox_ID.Tag.ToString();
                        db.T_Contracts.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.Contract_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.Contract_No == 0 || string.IsNullOrEmpty(data_this.Contract_ID) || !ifOkDelete)
            {
                return;
            }
            T_Emp returned = db.SelectContractNoByReturnNo(DataThis.Contract_No);
            int num;
            if (returned != null)
            {
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف العقد .. لانه مرتبط باحد الموظفين" : "can not be deleted This Job .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.ContractEmp(DataThis.Contract_No);
            try
            {
                db.T_Contracts.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_Contract")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["Contract_No"].Width = 120;
            panel.Columns["Contract_No"].Visible = columns_Names_visible["Contract_No"].IfDefault;
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير العقـــود");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_Contract.Contract_No as No, T_Contract.NameA as NmA, T_Contract.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
