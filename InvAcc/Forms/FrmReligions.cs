...
{ 
 panelEx2.Text=   (arln == 0 ? "  Click to collapse  " : "  Click to collapse") ; label40.Text=   (arln == 0 ? "  إسم الديانة - انجليزي :  " : "  Religion name - English:") ; label36.Text=   (arln == 0 ? "  إسم الديانة - عربـــي :  " : "  Religion name - Arabic:") ; label38.Text=   (arln == 0 ? "  الرمــــز :  " : "  Code:") ; superTabControl_Main1.Text=   (arln == 0 ? "  superTabControl3  " : "  superTabControl3") ; Button_Close.Text=   (arln == 0 ? "  إغلاق  " : "  Close") ; Button_Search.Text=   (arln == 0 ? "  بحث  " : "  Search") ; Button_Delete.Text=   (arln == 0 ? "  حذف  " : "  delete") ; Button_Save.Text=   (arln == 0 ? "  حفظ  " : "  save") ; Button_Add.Text=   (arln == 0 ? "  إضافة  " : "  addition") ; superTabControl_Main2.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; Button_First.Text=   (arln == 0 ? "  الأول  " : "  the first") ; Button_Prev.Text=   (arln == 0 ? "  السابق  " : "  the previous") ; lable_Records.Text=   (arln == 0 ? "  ---  " : "  ---") ; Button_Next.Text=   (arln == 0 ? "  التالي  " : "  next one") ; Button_Last.Text=   (arln == 0 ? "  الأخير  " : "  the last one") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; DGV_Main.Text=   (arln == 0 ? "  جميــع السجــــلات  " : "  All records") ; DGV_Main.Text=   (arln == 0 ? "    " : "    ") ; panelEx3.Text=   (arln == 0 ? "  Fill Panel  " : "  Fill Panel") ; superTabControl_DGV.Text=   (arln == 0 ? "  superTabControl1  " : "  superTabControl1") ; textBox_search.Text=   (arln == 0 ? "  ...  " : "  ...") ; Button_ExportTable2.Text=   (arln == 0 ? "  تصدير  " : "  Export") ; /*Button_PrintTable.Text=   (arln == 0 ? "  طباعة  " : "  Print") ; */ToolStripMenuItem_Rep.Text=   (arln == 0 ? "  إظهار التقرير  " : "  Show report") ; ToolStripMenuItem_Det.Text=   (arln == 0 ? "  إظهار التفاصيل  " : "  Show details") ; Text = "كــــــرت الديانات";this.Text=   (arln == 0 ? "  كــــــرت الديانات  " : "  I hate religions") ;}
...
#pragma warning disable CS0414 // The field 'FrmReligions.FlagUpdate' is assigned but its value is never used
        private string FlagUpdate = "";
#pragma warning restore CS0414 // The field 'FrmReligions.FlagUpdate' is assigned but its value is never used
       ...
       // private IContainer components = null;
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_Religion> list = db.FillReligion_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_Religion> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_Religion";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_Religions.Select((T_Religion item) => new
            {
                Code = item.Religion_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmReligions));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
                ...
                textBox_Note.WatermarkText = "ملاحظـــــــات الديانة";
                Text = "كرت الــديانات";
            }
            else
            {
        ...
        private void FrmCity_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    columns_Names_visible.Add("Religion_No", new ColumnDictinary("رقم الديانة", "Religions No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_Religion newData = db.ReligionEmp(no);
                if (newData == null || newData.Religion_No == 0 || string.IsNullOrEmpty(newData.Religion_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.Religion_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.Religion_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_Religion value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.Religion_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_Religion GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.Religion_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.Religion_ID = textBox_ID.Tag.ToString();
                        db.T_Religions.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.Religion_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.Religion_No == 0 || string.IsNullOrEmpty(data_this.Religion_ID) || !ifOkDelete)
            {
                return;
            }
            T_Emp returned = db.SelectReligionNoByReturnNo(DataThis.Religion_No);
            int num;
            if (returned != null)
            {
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف الديانة .. لانه مرتبط باحد الموظفين" : "can not be deleted This Job .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.ReligionEmp(DataThis.Religion_No);
            try
            {
                db.T_Religions.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_Religion")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["Religion_No"].Width = 120;
            panel.Columns["Religion_No"].Visible = columns_Names_visible["Religion_No"].IfDefault;
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير الديــــانات");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_Religion.Religion_No as No, T_Religion.NameA as NmA, T_Religion.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
