    ...
    public partial class FrmTreatment : Form
    {
        ...
        public T_User Permmission
        {
            ...
            set
            {
                ...
                {
                    permission = value;
                    if (!VarGeneral.TString.ChkStatShow(value.Emp_FilStr, 49))
                    {
                        IfAdd = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.Emp_FilStr, 50))
                    {
                        CanEdit = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.Emp_FilStr, 51))
                    {
                        IfDelete = false;
                    }
                    else
        ...
        public void Button_Search_Click(object sender, EventArgs e)
        {
            ...
            VarGeneral.SFrmTyp = "T_treatment";
            frm.TopMost = true;
            frm.ShowDialog();
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_treatment> list = db.FillTreatment_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_treatment> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_treatment";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_treatments.Select((T_treatment item) => new
            {
                Code = item.treatment_No + string.Empty
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmTreatment));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
            {
        ...
        private void RibunButtons()
        {
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
            {
                ...
                textBox_Note.WatermarkText = "ملاحظـــــــات المعاملة";
                Text = "كــــــرت المعـــاملات";
            }
            else
            {
        ...
        private void FrmTreatment_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    columns_Names_visible.Add("treatment_No", new ColumnDictinary("رقم المعاملة", "treatment No", ifDefault: true, string.Empty));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_treatment newData = db.TreatmentEmp(no);
                if (newData == null || newData.treatment_No == 0 || string.IsNullOrEmpty(newData.treatment_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.treatment_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.treatment_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
        ...
        public void SetData(T_treatment value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.treatment_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_treatment GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.treatment_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.treatment_ID = textBox_ID.Tag.ToString();
                        db.T_treatments.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.treatment_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this == null || data_this.treatment_No == 0 || string.IsNullOrEmpty(data_this.treatment_ID) || !ifOkDelete)
            {
                return;
            }
            T_Commentary returned = db.SelectTreatmentByReturnNo(DataThis.treatment_No);
            int num;
            if (returned != null)
            {
                int? treatmentNo = returned.treatmentNo;
                num = ((treatmentNo.Value == 0 && treatmentNo.HasValue) ? 1 : 0);
            }
            else
            {
                num = 1;
            ...
            if (num == 0)
            {
                MessageBox.Show((LangArEn == 0) ? "لايمكن حذف المعاملة .. لانه مرتبط باحد معاملات التعقيب للموظف" : "can not be deleted This Treatment .. because it is linked to one of the employee", VarGeneral.ProdectNam, MessageBoxButtons.OK, MessageBoxIcon.Hand);
                return;
            }
            data_this = db.TreatmentEmp(DataThis.treatment_No);
            try
            {
                db.T_treatments.DeleteOnSubmit(DataThis);
                db.SubmitChanges();
            }
            catch (SqlException)
            {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_treatment")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["treatment_No"].Width = 120;
            panel.Columns["treatment_No"].Visible = columns_Names_visible["treatment_No"].IfDefault;
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير المعــامــلات");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Fields = " T_treatment.treatment_No as No, T_treatment.NameA as NmA, T_treatment.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
