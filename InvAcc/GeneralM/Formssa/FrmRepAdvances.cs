...
{
    public partial class FrmRepAdvances : Form
    {
        ...
        private void FrmRepAdvances_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmRepAdvances));
                if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
                {
                ...
                FillGrid();
                FillCombo();
                List<string> q = (from t in db.T_Advances
                                  orderby t.ResolutionDate
                                  select t.ResolutionDate).ToList();
                if (q.Count() == 0)
                {
                    dateTimeInput_StartAdvancFrom.Text = string.Empty;
                    dateTimeInput_StartAdvancTo.Text = string.Empty;
                }
                else
                {
                    dateTimeInput_StartAdvancFrom.Text = Convert.ToDateTime(q.First()).ToString("yyyy/MM/dd");
                    dateTimeInput_StartAdvancTo.Text = Convert.ToDateTime(q.Last()).ToString("yyyy/MM/dd");
                }
            }
        ...
        public void FillCombo()
        {
            ...
            comboBox_AdvancStatus.Items.Add((LangArEn == 0) ? "المسد\u0651د فقط" : "Paid");
            comboBox_AdvancStatus.Items.Add((LangArEn == 0) ? "الغير مسد\u0651د فقط" : "Not Paid");
        ...
        private void FrmRepAdvances_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                SendKeys.Send("{Tab}");
        ...
        private void FrmRepAdvances_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(InvAcc.Forms.FrmRepAdvances));
            ...
            base.Name = "FrmRepAdvances";
            base.Load += new System.EventHandler(FrmRepAdvances_Load);
            base.KeyPress += new System.Windows.Forms.KeyPressEventHandler(FrmRepAdvances_KeyPress);
            base.KeyDown += new System.Windows.Forms.KeyEventHandler(FrmRepAdvances_KeyDown);
