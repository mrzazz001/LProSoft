...
{
    public partial class FrmRepReward : Form
    {
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(InvAcc.Forms.FrmRepReward));
            ...
            base.Name = "FrmRepReward";
            base.ShowIcon = false;
            base.Load += new System.EventHandler(FrmRepReward_Load);
            base.KeyPress += new System.Windows.Forms.KeyPressEventHandler(FrmRepReward_KeyPress);
            base.KeyDown += new System.Windows.Forms.KeyEventHandler(FrmRepReward_KeyDown);
        ...
        private void FrmRepReward_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmRepReward));
                if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
                {
                ...
                RibunButtons();
                FillGrid();
                List<string> q = (from t in db.T_Rewards
                                  orderby t.RewardDate
                                  where t.RewardDate != string.Empty
                                  select t.RewardDate).ToList();
                if (q.Count == 0)
                {
                    dateTimeInput_RewardFrom.Text = string.Empty;
                    dateTimeInput_RewardTo.Text = string.Empty;
                }
                else
                {
                    dateTimeInput_RewardFrom.Text = Convert.ToDateTime(q.First()).ToString("yyyy/MM/dd");
                    dateTimeInput_RewardTo.Text = Convert.ToDateTime(q.Last()).ToString("yyyy/MM/dd");
                }
            }
        ...
        private void FrmRepReward_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                SendKeys.Send("{Tab}");
        ...
        private void FrmRepReward_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void ButOk_Click(object sender, EventArgs e)
        {
            ...
            {
                VarGeneral.DebLog.writeLog("FrmRepReward_Load:", error, enable: true);
                MessageBox.Show(error.Message);
            }
        }
        private string GetSqlWhere()
        {
            ...
            {
                QStr = QStr + " AND ( RewardDate >= '" + Convert.ToDateTime(dateTimeInput_RewardFrom.Text).ToString("yyyy/MM/dd") + "' ) ";
            }
            if (dateTimeInput_RewardTo.Text != "    /  /" && dateTimeInput_RewardTo.Text.Length == 10)
            {
                QStr = QStr + " AND ( RewardDate <= '" + Convert.ToDateTime(dateTimeInput_RewardTo.Text).ToString("yyyy/MM/dd") + "' ) ";
            }
            return QStr;
        }
