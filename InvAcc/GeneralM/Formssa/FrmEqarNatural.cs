...
{
    public partial class FrmEqarNatural : Form
    {
        ...
        private T_EqarNatural data_this;
        ...
        public T_User Permmission
        {
            ...
            set
            {
                ...
                {
                    permission = value;
                    if (!VarGeneral.TString.ChkStatShow(value.Eqar_FilStr, 13))
                    {
                        IfAdd = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.Eqar_FilStr, 14))
                    {
                        CanEdit = false;
                    }
                    else
                    ...
                    if (!VarGeneral.TString.ChkStatShow(value.Eqar_FilStr, 15))
                    {
                        IfDelete = false;
                    }
                    else
        ...
        public void Button_Search_Click(object sender, EventArgs e)
        {
            ...
            VarGeneral.SFrmTyp = "T_EqarNatural";
            frm.TopMost = true;
            frm.ShowDialog();
            if (frm.SerachNo != "")
            {
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_EqarNatural> list = db.FillEqarNatural_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_EqarNatural> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_EqarNatural";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_EqarNaturals.Select((T_EqarNatural item) => new
            {
                Code = item.EqarNatural_No + ""
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmEqarNatural));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
            {
        ...
        private void RibunButtons()
        {
            ...
            else
            {
                ...
                Text = "Nature of the property Card";
            }
        }
        private void FrmEqarNatural_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmEqarNatural));
                ...
                {
                    columns_Names_visible.Add("EqarNatural_No", new ColumnDictinary("الرقـــــم", "No", ifDefault: true, ""));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_EqarNatural newData = db.EqarNaturalEmp(no);
                if (newData == null || newData.EqarNatural_No == 0 || string.IsNullOrEmpty(newData.EqarNatural_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.EqarNatural_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.EqarNatural_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
                ...
                try
                {
                    if (data_this.EqarNatural_No == 1 || data_this.EqarNatural_No == 2)
                    {
                        IfDelete = false;
                        CanEdit = false;
                    }
        ...
        public void SetData(T_EqarNatural value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.EqarNatural_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_EqarNatural GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.EqarNatural_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.EqarNatural_ID = textBox_ID.Tag.ToString();
                        db.T_EqarNaturals.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.EqarNatural_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this != null && data_this.EqarNatural_No != 0 && !string.IsNullOrEmpty(data_this.EqarNatural_ID) && ifOkDelete)
            {
                data_this = db.EqarNaturalEmp(DataThis.EqarNatural_No);
                try
                {
                    db.T_EqarNaturals.DeleteOnSubmit(DataThis);
                    db.SubmitChanges();
                }
                catch (SqlException)
                {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_EqarNatural")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["EqarNatural_No"].Width = 120;
            panel.Columns["EqarNatural_No"].Visible = columns_Names_visible["EqarNatural_No"].IfDefault;
            panel.ReadOnly = true;
        }
        ...
        private void Button_ExportTable2_Click(object sender, EventArgs e)
        {
            try
            {
                ExportExcel.ExportToExcel(DGV_Main, "تقرير أنواع العقار");
            }
            catch
            {
            }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string Fields = "";
                Fields = " T_EqarNatural.EqarNatural_No as No, T_EqarNatural.NameA as NmA, T_EqarNatural.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
                _RepShow.Rule = "";
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(InvAcc.Forms.FrmEqarNatural));
            ...
            panelEx2.MinimumSize = new System.Drawing.Size(649, 205);
            ...
            base.Name = "FrmEqarNatural";
            base.Load += new System.EventHandler(FrmEqarNatural_Load);
