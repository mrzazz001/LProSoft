...
{
    public partial class FrmEqarTyp : Form
    {
        ...
        {
            DGV_Main.PrimaryGrid.VirtualMode = false;
            List<T_EqarTyp> list = db.FillEqarTyp_2(textBox_search.TextBox.Text).ToList();
        ...
        public void FillHDGV(IEnumerable<T_EqarTyp> new_data_enum)
        {
            ...
            DGV_Main.PrimaryGrid.DataMember = "T_EqarTyp";
        ...
        {
            PKeys.Clear();
            var qkeys = db.T_EqarTyps.Select((T_EqarTyp item) => new
            {
                Code = item.EqarTyp_No + string.Empty
            });
            int count = 0;
        ...
        {
            ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmEqarTyp));
            if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
            {
        ...
        private void FrmEqarTyp_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmEqarTyp));
        ...
        private void textBox_ID_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                T_EqarTyp newData = db.EqarTypEmp(no);
                if (newData == null || newData.EqarTyp_No == 0 || string.IsNullOrEmpty(newData.EqarTyp_ID))
                {
                    if (!Button_Add.Visible || !Button_Add.Enabled)
                    {
                        ...
                        try
                        {
                            textBox_ID.Text = data_this.EqarTyp_No.ToString();
                        }
                        catch
                        {
                        }
                ...
                {
                    DataThis = newData;
                    int indexA = PKeys.IndexOf(string.Concat(newData.EqarTyp_No));
                    indexA++;
                    TextBox_Index.InputTextChanged -= TextBox_Index_InputTextChanged;
                ...
                try
                {
                    if (data_this.EqarTyp_No == 1 || data_this.EqarTyp_No == 2 || data_this.EqarTyp_No == 3)
                    {
                        IfDelete = false;
                        CanEdit = false;
                    }
        ...
        public void SetData(T_EqarTyp value)
        {
            try
            {
                State = FormState.Saved;
                ...
                textBox_ID.Tag = value.EqarTyp_ID;
                Permmission = dbc.Get_PermissionID(VarGeneral.UserID);
            }
            catch (Exception error)
            {
        ...
        private T_EqarTyp GetData()
        {
            textBox_ID.Focus();
            try
            {
                data_this.EqarTyp_No = int.Parse(textBox_ID.Text);
            }
            catch
            {
            }
        ...
        private void Button_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (State == FormState.New)
                {
                    ...
                    try
                    {
                        data_this.EqarTyp_ID = textBox_ID.Tag.ToString();
                        db.T_EqarTyps.InsertOnSubmit(data_this);
                        db.SubmitChanges();
                    }
                ...
                State = FormState.Saved;
                RefreshPKeys();
                TextBox_Index.TextBox.Text = string.Concat(PKeys.IndexOf(string.Concat(data_this.EqarTyp_No)) + 1);
                SetReadOnly = true;
            }
            catch (Exception error)
            {
        ...
        private void Button_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (data_this != null && data_this.EqarTyp_No != 0 && !string.IsNullOrEmpty(data_this.EqarTyp_ID) && ifOkDelete)
            {
                data_this = db.EqarTypEmp(DataThis.EqarTyp_No);
                try
                {
                    db.T_EqarTyps.DeleteOnSubmit(DataThis);
                    db.SubmitChanges();
                }
                catch (SqlException)
                {
        ...
        private void DGV_Main_DataBindingComplete(object sender, GridDataBindingCompleteEventArgs e)
        {
            ...
            if (dataMember != null && dataMember == "T_EqarTyp")
            {
        ...
        private void PropBranchPanel(GridPanel panel)
        {
            ...
            panel.Columns["EqarTyp_No"].Width = 120;
            panel.Columns["EqarTyp_No"].Visible = columns_Names_visible["EqarTyp_No"].IfDefault;
            panel.ReadOnly = true;
        }
        ...
        public void Button_Print_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Fields = " T_EqarTyp.EqarTyp_No as No, T_EqarTyp.NameA as NmA, T_EqarTyp.NameE as NmE,(select T_SYSSETTING.LogImg from T_SYSSETTING) as LogImg ";
        ...
        private void InitializeComponent()
        {
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(InvAcc.Forms.FrmEqarTyp));
            ...
            base.Name = "FrmEqarTyp";
            base.Load += new System.EventHandler(FrmEqarTyp_Load);
