...
{
    public partial class FrmRepCalls : Form
    {
        ...
        private void FrmRepCalls_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmRepCalls));
                if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == "")
                {
                ...
                RibunButtons();
                FillGrid();
                List<string> q = (from t in db.T_CallPhones
                                  orderby t.PhoneDate
                                  select t.PhoneDate).ToList();
                if (q.Count() == 0)
                {
        ...
        private void FrmRepCalls_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                SendKeys.Send("{Tab}");
        ...
        private void FrmRepCalls_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private string GetSqlWhere()
        {
            ...
            {
                QStr = QStr + " AND ( PhoneDate >= '" + Convert.ToDateTime(dateTimeInput_AddFrom.Text).ToString("yyyy/MM/dd") + "' ) ";
            }
            if (dateTimeInput_AddTo.Text != "    /  /" && dateTimeInput_AddTo.Text.Length == 10)
            {
                QStr = QStr + " AND ( PhoneDate <= '" + Convert.ToDateTime(dateTimeInput_AddTo.Text).ToString("yyyy/MM/dd") + "' ) ";
            }
            return QStr;
        }
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(InvAcc.Forms.FrmRepCalls));
            ...
            base.Name = "FrmRepCalls";
            base.Load += new System.EventHandler(FrmRepCalls_Load);
            base.KeyPress += new System.Windows.Forms.KeyPressEventHandler(FrmRepCalls_KeyPress);
            base.KeyDown += new System.Windows.Forms.KeyEventHandler(FrmRepCalls_KeyDown);
