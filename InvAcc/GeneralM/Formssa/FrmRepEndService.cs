...
{
    public partial class FrmRepEndService : Form
    {
        ...
        private ComboBoxEx comboBox_LiquStatus;
        ...
        private void RibunButtons()
        {
            ...
            else
            {
                ...
                Text = "End Service Report";
            }
        }
        private void FrmRepEndService_Load(object sender, EventArgs e)
        {
            try
            {
                ComponentResourceManager resources = new ComponentResourceManager(typeof(FrmRepEndService));
                if (VarGeneral.CurrentLang.ToString() == "0" || VarGeneral.CurrentLang.ToString() == string.Empty)
                {
                ...
                FillGrid();
                FillCombo();
                var q = (from t in db.T_EndServices
                         orderby t.DateFilter
                         where t.DateFilter != string.Empty
                         select new
                         {
                             t.DateFilter
                         }).ToList();
                if (q.Count == 0)
                {
                    dateTimeInput_StartVacationFrom.Text = string.Empty;
                    dateTimeInput_StartVacationTo.Text = string.Empty;
                }
                else
                {
                    dateTimeInput_StartVacationFrom.Text = Convert.ToDateTime(q.First().DateFilter).ToString("yyyy/MM/dd");
                    dateTimeInput_StartVacationTo.Text = Convert.ToDateTime(q.Last().DateFilter).ToString("yyyy/MM/dd");
                }
            }
        ...
        {
            List<T_LiquidationTyp> listT_LiquidationTyps = new List<T_LiquidationTyp>(db.T_LiquidationTyps.Select((T_LiquidationTyp item) => item).ToList());
            comboBox_LiquStatus.DataSource = listT_LiquidationTyps;
            comboBox_LiquStatus.DisplayMember = ((LangArEn == 0) ? "NameA" : "NameE");
            comboBox_LiquStatus.ValueMember = "LiquidationT_No";
            comboBox_LiquStatus.SelectedIndex = -1;
        }
        ...
        private void FrmRepEndService_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                SendKeys.Send("{Tab}");
        ...
        private void FrmRepEndService_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private string GetSqlWhere()
        {
            ...
            {
                QStr = QStr + " AND ( DateFilter >= '" + Convert.ToDateTime(dateTimeInput_StartVacationFrom.Text).ToString("yyyy/MM/dd") + "' ) ";
            }
            if (dateTimeInput_StartVacationTo.Text != "    /  /" && dateTimeInput_StartVacationTo.Text.Length == 10)
            {
                QStr = QStr + " AND ( DateFilter <= '" + Convert.ToDateTime(dateTimeInput_StartVacationTo.Text).ToString("yyyy/MM/dd") + "' ) ";
            }
            if (comboBox_LiquStatus.SelectedIndex != -1)
            {
                QStr = QStr + " AND T_EndService.CauseLiquidation = " + int.Parse(comboBox_LiquStatus.SelectedValue.ToString());
            }
            return QStr;
        }
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(InvAcc.Forms.FrmRepEndService));
            ...
            comboBox_LiquStatus = new DevComponents.DotNetBar.Controls.ComboBoxEx();
            ...
            panel1.Controls.Add(comboBox_LiquStatus);
            ...
            comboBox_LiquStatus.AutoCompleteSource = System.Windows.Forms.AutoCompleteSource.ListItems;
            comboBox_LiquStatus.DisplayMember = "Text";
            comboBox_LiquStatus.DrawMode = System.Windows.Forms.DrawMode.OwnerDrawFixed;
            comboBox_LiquStatus.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
            comboBox_LiquStatus.FormattingEnabled = true;
            resources.ApplyResources(comboBox_LiquStatus, "comboBox_LiquStatus");
            comboBox_LiquStatus.Name = "comboBox_LiquStatus";
            comboBox_LiquStatus.Style = DevComponents.DotNetBar.eDotNetBarStyle.StyleManagerControlled;
            ...
            base.Name = "FrmRepEndService";
            base.Load += new System.EventHandler(FrmRepEndService_Load);
            base.KeyPress += new System.Windows.Forms.KeyPressEventHandler(FrmRepEndService_KeyPress);
            base.KeyDown += new System.Windows.Forms.KeyEventHandler(FrmRepEndService_KeyDown);
